...
if __name__ == '__main__':
    ...
    def print_noticias(index, response):
        if response.status_code == 200:
            ...
            if bs:
                articles = bs.find('div', {'class': 'csl-inner csl-hot'}).find_all('article')
    ...
    for i in range(1, 6):
        ...
        index = print_noticias(index, response)
    for i in range(1, 6):
        response = requests.get("https://www.valenciaextra.com/sucessos?_page=" + str(i), headers={'User-Agent': 'Mozilla/5.0'})
        index = print_noticias(index, response)
    for i in range(1, 6):
